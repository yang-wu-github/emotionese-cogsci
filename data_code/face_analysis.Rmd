...
d <- d_raw %>%
  ...
  mutate(valence=fct_relevel(valence, "positive", "negative"),
         ...
         Category = case_when(category == "excitement" ~ "Colorful Bubbles",
                              category == "affection" ~ "Stuffed Animals",
                              category == "craving" ~ "Yummy Snacks",
                              category == "disgust" ~ "Rotten Fruits",
                              category == "fear" ~ "Dangerous Fire",
                              category == "sadness" ~ "Broken Toys"),
         Category=factor(Category, levels = c("Colorful Bubbles", "Stuffed Animals", "Yummy Snacks",
                                              "Broken Toys", "Dangerous Fire", "Rotten Fruits")),
...
fig <- ggplot(ms, aes(x=Condition, y = mean, group=Valence, color=Valence)) +
  ...
  ylab("Valence of Emotional Expression")
fig
#ggsave(here("figures/230131fig_by_valence.pdf"), width = 5, height = 3)
```
...
fig_cat <- ggplot(ms_cat, aes(x=Condition, y = mean, fill=Category)) +
  ...
  ylab("Valence of Emotional Expression")
fig_cat
#ggsave(here("figures/230131fig_by_category.pdf"), width = 6, height = 3)
ggarrange(fig, fig_cat, labels = c("A", "B"), widths = c(1.5, 2.5), ncol = 2, nrow = 1)
ggsave(here("figures/230131fig_combined.png"), width = 10, height = 3.3)
```
# Analyze data
```{r}
